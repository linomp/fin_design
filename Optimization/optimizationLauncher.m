...
function [population, best_candidates] = optimizationLauncher(problem)  
    ...
    problem.pc = 1;%0.50;              % crossover probability
    problem.mutation_rate = 0.8;%0.65;   % mutation method proposed in Haupt (2004)   
    problem.mutProb = 0.20;         % for gaussian mutation
    problem.mutSTD = 1e-2;             % std. deviation for gaussian mutation
   ...
   %----------------------------- GA EXECUTION ------------------------------
    ...
    % GO!
    [population, best_candidates] = theGARoutine(problem, population);  
    ...
    fprintf('\nCurrent Generation: %d', problem.current_generation); 
    fprintf('\nBest objective function value found: %d \n', population.bestIndividual.fitness);

    % Show best individual at end of GA, save last frame to video
    %decodeChromosome(population.bestIndividual);  
    problem.plotEvolution(population.pool, population.bestIndividual);


    if(~problem.plotOpts.quiet) 
        if (problem.plotOpts.plot_full_results)
            close(problem.V_GA);
        end
        if (problem.plotOpts.plot_gbm_path) 
            close(problem.V_GBM);
        end
    end  
    disp('... Done ...')
