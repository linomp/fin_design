...
function [population, candidates] = theGARoutine(problem, population)  
    ...
    for i = 1 : population.popSize  
        population.theIndividuals{i}.fitness =  (problem.objective_func(population.theIndividuals{i}.chrom, problem.bc));
    ...
    for current_generation = 2:problem.maxGens   
       ...
       for i = 1:length(population.pool)   
           population.pool{i}.fitness =  problem.objective_func(population.pool{i}.chrom,problem.bc); 
           problem.ff_evals = problem.ff_evals + 1; 
       end  
    ...
    if problem.plotOpts.quiet
       ...
       if(current_generation > 2)
           figure(2)
           hold on
           loglog(1:length(bests), bests, 'g-', 'linewidth', 2);
           loglog(1:length(avgs), avgs, 'r--', 'linewidth', 2);
